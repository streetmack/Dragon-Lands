	...
	while(k!=L'0')
	{
		...
		switch(k) // Варианты диалога
		{
			...
			case L'3':
			{
				...
				PrintLog(L"Лучше возьми зелий с запасом мало ли на кого наткнешся! Ты не думай что ", FG_WHITE);
				PrintLog(L"я тебе впарить чтото пытаюсь,просто если не хватит будешь потом валяться ",FG_WHITE);
				PrintLog(L"в канаве, а золотишко достанится какому нибудь разбойнику.", FG_WHITE);
				PrintLog(L"Дак лучше уж мне) Я всетаки доброе дело делаю", FG_WHITE);
				break;
			}
			case L'4': // Квест
			{
	...
	while(k!=L'0')
	{
		...
		switch(k)
		{
			...
			case L'3':
			{
				...
				PrintLog(L"Чтобы сразиться с ним, тебе потребуется лучшее наряжение какое только сможешь найти (минимум 10 уровень).",FG_WHITE);
				break;
			}
			case L'4':
			{
	...
	while(k!=L'0')
	{
		...
		switch(k)
		{
			case L'1':
			{
				...
				PrintLog(L"О_о очередной герой?", FG_WHITE);
				PrintLog(L"Очень много прид.. ээ... славных героев хотят завалить дракона, но все оказываются в одном месте",FG_WHITE);
				PrintLog(L"Ну и я решил не пускать совсем зеленых туда",FG_WHITE);
				break;
			}
			case L'2':
			{
				PrintLog(L"Привратник говорит:",FG_WHITE|FG_INT);
				PrintLog(L"А оказались они все на кладбище",FG_WHITE);
				PrintLog(L"Земля проклята, и они восстали в виде нежити, бродят, пристают к туристам",FG_WHITE);
				PrintLog(L"Мы бы их всех уничтожили, но они блин постоянно оживают =(",FG_WHITE);
				break;
			}
			case L'3':
			{
			...
			case L'4':
			{
				...
				if(!g_player->bRelic)
				{
					...
					PrintLog(L"Там много монстров, и если ты выживешь, значит готов встретить дракона! ну почти)",FG_WHITE);
				}else{
					PrintLog(L"Квест выполнен!",FG_YELLOW|FG_INT);
					PrintLog(L"Замечательно, теперь ты можешь пройти. Возможно ты выживешь и возьми это, на всякий случай",FG_WHITE);
					PrintLog(L"Кстати какой у тебя рост ?", FG_WHITE);
					PrintLog(L"Да лан шучу, топай давай", FG_WHITE);
